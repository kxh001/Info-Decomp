...
from configs.eval_daam_itd_configs import parse_args_and_update_config
...
if __name__ == '__main__':
    ...
    print(f'sdm: {sdm_version} | result: attnmaps | dataset: {csv_name} | guide_scale: {guide_scale} | steps: {steps} |'
          f' logsnr_loc: {logsnr_loc} | logsnr_scale: {logsnr_scale} | clip: {clip}')
    ...
    with torch.no_grad():
        for batch_idx, batch in tqdm(enumerate(dataloader)):
            ...
            if steps == 1:
                logsnrs, _ = utils.one_step_test(loc=1.0, scale=2.0, clip=3.0, device=device)
            else:
                logsnrs, _ = utils.logistic_integrate(npoints=steps, loc=1.0, scale=2.0, clip=3.0, device=device)

            with torch.cuda.amp.autocast(dtype=torch.float16), torch.no_grad():
                with trace(pipe) as tc:
                    for logsnr in logsnrs:
                        z, _ = itd.noisy_channel(image_latents, logsnr)
                        noise_pred = itd.forward(z, itd.logsnr2t(logsnr.view(1)), cond=text_embeddings)

                    rec_img = latents_to_imgs(image_latents)[0]
